...
const BulkExportUI = (() => {
  ...
  function _defaultVoice() {
    // Try to pick up the global app voice setting if Reader is active
    try {
      const state = Reader?.getState?.();
      return state?.chosenVoice?._edgeVoice || 'en-US-AriaNeural';
    } catch (_) { return 'en-US-AriaNeural'; }
  }
  ...
  async function openConfig() {
    ...
    if (!_voices.length) {
      try {
        _voices = await window.sonara.tts.getVoices() || [];
      } catch (_) { _voices = []; }
    }
  ...
  function _renderConfigTable() {
    ...
    tbody.innerHTML = _selectedBooks.map(book => {
      ...
      const voiceOptions = _voices.map(v => {
        const selected = v.ShortName === voice ? ' selected' : '';
        return '<option value="' + _escHtml(v.ShortName) + '"' + selected + '>' + _escHtml(v.FriendlyName || v.ShortName) + '</option>';
      }).join('');

      const isAudio = ['mp3', 'm4a', 'ogg', 'm4b', 'aac'].includes((book.format || '').toLowerCase());
      const voiceCell = isAudio
        ? '<td><span style="color:var(--text-muted);font-size:11px">Re-encode only</span></td><td>—</td><td>—</td>'
        : '<td><select class="bulk-cfg-select" data-book-id="' + bookId + '" data-field="voice">' + voiceOptions + '</select></td>' +
          '<td><input type="range" class="bulk-cfg-mini-slider" min="0.5" max="2" step="0.1" value="' + speed + '" data-book-id="' + bookId + '" data-field="speed" oninput="BulkExportUI._onSliderChange(this)"/>' +
          '<span class="bulk-cfg-mini-val" id="bcs-speed-' + bookId + '">' + speed.toFixed(1) + 'x</span></td>' +
          '<td><input type="range" class="bulk-cfg-mini-slider" min="0.5" max="2" step="0.1" value="' + pitch + '" data-book-id="' + bookId + '" data-field="pitch" oninput="BulkExportUI._onSliderChange(this)"/>' +
          '<span class="bulk-cfg-mini-val" id="bcs-pitch-' + bookId + '">' + pitch.toFixed(1) + 'x</span></td>';

      return '<tr>' +
        '<td title="' + _escHtml(book.title) + '">' + _escHtml(book.title) + '</td>' +
        voiceCell +
      '</tr>';
    }).join('');

    // Wire voice select changes
    tbody.querySelectorAll('.bulk-cfg-select[data-field=voice]').forEach(sel => {
      sel.addEventListener('change', () => {
        const id = sel.dataset.bookId;
        if (!_voiceOverrides[id]) _voiceOverrides[id] = {};
        _voiceOverrides[id].voice = sel.value;
      });
    });
  }
